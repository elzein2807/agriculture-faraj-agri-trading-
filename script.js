...
/* ── IMAGE LOADER (preload → no broken icons; fallback gradient stays) ── */
function loadBg(el) {
  const url = el.dataset.bg;
  if (!url) return;
  const img = new Image();
  img.onload = () => { el.style.backgroundImage = `url("${url}")`; el.classList.add('loaded'); };
  img.onerror = () => { el.classList.add('img-failed'); };   /* fallback gradient remains visible */
  img.src = url;
}
...
/* ── HERO BACKGROUND ROTATION (every 2s, fade + zoom) ───────── */
(function () {
...
/* ── GENERIC ROTATORS (projects + gallery, every 2s) ────────── */
(function () {
  $$('.rot').forEach(rot => {
    ...
    slides[0].classList.add('on');
    /* slight phase offset so tiles don't all flip in unison */
...
/* ── 3D TILT ON PROJECT CARDS (pointer-fine only) ───────────── */
(function () {
